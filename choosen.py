...
class Chosen(object):
    ...
    def __init__(self, webdriver, select_id, nav_bar_height=50):
        ...
        chosen_id = (select_id + '_chzn').replace('-', '_')
        self.chosen = webdriver.find_element_by_id(chosen_id)
        self.chosen.input = self.chosen.find_element_by_css_selector('.chzn-single')
        self.chosen_drop = self.chosen.find_element_by_css_selector('.chzn-drop')
    ...
    def get_options(self, close=True):
        ...
        """
        self.open_select()
        results = self.chosen.find_elements_by_css_selector('.chzn-results li')
        if close:
            self.close_select()
        return results
    ...
    def get_options_text(self, close=True):
        ...
        results = [x.text for x in self.chosen.find_elements_by_css_selector('.chzn-results li')]
        if close:
            self.close_select()
        return results
    ...
    def get_selected_options(self, close=True):
        ...
        results = []
        for element in self.chosen.find_elements_by_css_selector('ul.chzn-results li.result-selected'):
    ...
    def select_by_search(self, choice_text):
        ...
        """
        self.open_select()
        search = self.chosen.find_element_by_css_selector('.chzn-search input')
